...
class TestNotificationManager:
    @pytest.fixture
    def notifier_config(self, request):
    ...
    def test_init_disabled(self, notification_manager, notifier_config):
        assert len(notification_manager._notifiers) == 1
        assert len(notification_manager._message_to_notifier_map) == 1

    @pytest.mark.parametrize("notifier_config", [""], indirect=True)
    def test_asa(self, notification_manager_one, notifier_config):
